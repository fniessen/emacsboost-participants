...
** Fusion de deux listes numérotées avec une macro Emacs

*** Objectif

Fusionner deux listes numérotées dans Emacs, en plaçant un élément de la seconde
liste juste en dessous de l’élément correspondant de la première. À partir de :
...
*** Solution proposée : créer une macro Emacs

L'idée est d'automatiser l’insertion de chaque élément de la deuxième liste
juste après le bon élément de la première en procédant comme suit :

1. Placer le curseur sur le premier élément de la liste d'exemples.
2. Créer une macro qui :
   - effectue une recherche arrière (C-r) du numéro de l’item courant, par
     exemple 1., 2., etc.,
   - une fois trouvée, insère une nouvelle ligne juste après,
   - colle ou insère le contenu de l’exemple (celui sur lequel on a lancé la
     macro),
   - se replace sur le prochain élément de la liste d’exemples (le suivant en
     bas).
3. Répéter cette macro pour les trois éléments (ou utiliser C-u 3 pour la
   répéter 3 fois automatiquement).
...
a        & this & that & result \\
best     & other-thing & something else & other \\
some-var & last & a bigger description & here \\
